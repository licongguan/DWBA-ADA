...
def train(cfg):
    global mIoUs
    logger = logging.getLogger("AL-DWACA.trainer")
    ...
    for batch_index, (src_data, tgt_data) in enumerate(zip(src_train_loader, tgt_train_loader)):
        ...
        if iteration == cfg.SOLVER.MAX_ITER or iteration % cfg.SOLVER.CHECKPOINT_PERIOD == 0 or iteration in cfg.ACTIVE.SELECT_ITER:
            ...
            # eval
            mIoUs = test(cfg, iteration, feature_extractor, classifier)
...
def test(cfg, iteration, feature_extractor, classifier):
    logger = logging.getLogger("AL-DWACA.trainer")
    logger.info("Start testing")
    # read now model
    now_epoch_model = os.path.join(cfg.OUTPUT_DIR, "model_iter{:06d}.pth".format(iteration))
    if os.path.exists(now_epoch_model):
        logger.info("Loading checkpoint from {}".format(now_epoch_model))
        checkpoint = torch.load(now_epoch_model, map_location=torch.device('cpu'))
    else:
        logger.info("Loading checkpoint from model_iter010000.pth")
        checkpoint = torch.load(os.path.join(cfg.OUTPUT_DIR, "model_iter010000.pth"), map_location=torch.device('cpu'))
    ...
    for batch in tqdm(test_loader):
        ...
        pred = inference(feature_extractor, classifier, x, y, True)  # 1 19 1024 2048
        output = pred.max(1)[1]
    ...
    iou_class = intersection_meter.sum / (union_meter.sum + 1e-10)  # ndarray
    mIoU = np.mean(iou_class)

    logger.info('Val result: mIoU: {:.4f}.'.format(mIoU))
    for i in range(cfg.MODEL.NUM_CLASSES):
        logger.info('{} {} iou: {:.4f}.'.format(i, test_data.trainid2name[i], iou_class[i]))

    # to list
    mIoUs = iou_class.tolist()
...
def main():
    ...
    logger = setup_logger("AL-DWACA", output_dir, 0)
    logger.info(args)
